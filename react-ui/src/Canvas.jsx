...
function Canvas(props) {
    ...
    const [track_analysis, setAnalysis] = useState({})
    ...
    useEffect(() => {
        ...
        if (current_track.id) {
            ...
            let query = 'https://api.spotify.com/v1/audio-analysis/' + current_track.id;
            fetch(query, {
                method: "GET",
                headers: {
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(track_analysis)) {
            // drawAnalysis(track_analysis)
            console.log(track_analysis)
        }
    }, [track_analysis])
